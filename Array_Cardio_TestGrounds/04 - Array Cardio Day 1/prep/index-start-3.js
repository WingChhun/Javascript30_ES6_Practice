...
const mappedInventors = inventors.map(
...
const sortedNames = people.sort((a, b) => {
  ...
  return name[name.length - 1] > nameB[nameB.length - 1] ? 1 : -1;
});
...
const summed = data.reduce((accumulator, data) => {
  if (!accumulator[data]) {
    accumulator[data] = 0;
  }
